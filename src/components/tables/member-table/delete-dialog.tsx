    ...
    startDeleteTransition(() => {
      toast.promise(client.delete(`/members?ids=${memberIds.join(',')}`), {
        ...
        error: (error) => {
          ...
          return t('delete_record_failed', { name: t('member').toLowerCase() });
        }
      });
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="top-[200px] sm:max-w-[425px]">
        {members.length > 1 && <p>{t('are_you_sure_to_delete_these_records', { name: t('members').toLowerCase() })}</p>}

        {members.length === 1 && <p>{t('are_you_sure_to_delete_this_record', { name: t('member').toLowerCase() })}</p>}
